...
已修复同一 Go 进程内并发启动多个 Firefox 实例时，`AutoPortEnabled(true)` 可能把多个实例分配到同一个调试端口、最终复用到同一个浏览器的问题。

当前行为：

- 同一 Go 进程内并发启动时，自动端口分配会先做进程内唯一预留
- 启动失败或连接失败时，会自动换一个新端口继续重试
- 浏览器退出后，端口会释放，后续实例可以再次复用
- 如果你显式调用 `WithAddress("127.0.0.1:xxxx")`，则仍以显式地址为准，不走自动端口分配

这次修复后，`AutoPortEnabled(true)` 不再只是“启动前扫描空闲端口”，而是具备了**同进程并发安全**和**退出后可复用**的行为。
