...
func (p *FirefoxBase) Navigate(targetURL string, wait string) error {
	...
	navigateWait := wait
	if wait == "complete" && p.intercept != nil && p.intercept.Active() {
	...
	if navigateErr == nil && navigateWait != wait && wait == "complete" {
		if err := p.WaitReadyState("complete", p.pageLoadTimeout()); err != nil {
			return err
		}
	}
